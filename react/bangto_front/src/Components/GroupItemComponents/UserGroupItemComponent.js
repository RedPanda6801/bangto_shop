...
const UserGroupItemComponent = () => 
{
    ...
    const handleLeftTime = () => {
        ...
        return `${day}일 ${hour}시간 ${String(minutes).padStart(2, '0')}분 ${String(seconds).padStart(2, '0')}초`;
    };

    const handleGetPeriod = async () => {
        try{
            const response = await axios.get(`${process.env.REACT_APP_BACKEND_SERVER_PORT}/group-buy/current-event`,
                {   
    ...
    const handleGroupItemList = async () => {
        axios.get(`${process.env.REACT_APP_BACKEND_SERVER_PORT}/group-item/event/get-list/${period.id}`,
            {   
                withCredentials: true,
                headers: {"Authorization": `Bearer ${localStorage.getItem("token")}`}
        }).then((res)=> {
                ...
                :<>
                    ...
                    <div className="box_Slide">
                        <div className="box_item box_slider">
                            <div className="slider_track" style={{justifyContent : "center"}}>
                                <img
                    ...
                    <div>
                        {
                            ...
                            <div style={{textAlign : 'center'}}>
                                <h3 style={{color : "red", fontSize : "bold"}}>{leftTime}까지!</h3><>얼마 남지 않았어요!</></div>
                    ...
                    <div>
                        ...
                        <div className="group_item_list">
                            {
                                groupItemList && groupItemList.length > 0 ? groupItemList.map((groupInfo)=>
                                    <div className="group_Item" onClick={()=> setSelectedGroupItem(groupInfo)}>
                                        <img className="group_item_img" src={`${process.env.REACT_APP_IMG_PUBLIC_URI}/03_upload/${groupInfo.item.img.split("/")[0]}`}></img>    
                                        <div>물품 이름: {groupInfo.item.title}</div>
                                        <div>물품 가격: {groupInfo.item.price}</div>
                                        <div>물품 카테고리: {CategoryType[groupInfo.item.category]}</div>
                                    </div>
                                )
