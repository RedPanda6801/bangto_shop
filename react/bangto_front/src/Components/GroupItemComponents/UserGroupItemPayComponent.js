...
const UserGroupItemPayComponent = () => 
{
    ...
    const handleDeliverCost = () => {
        let cost = 0;
        const price = Math.round((payItem.item.price + payItem.option.addPrice) * 0.9 * amount);
    ...
    const handleTotalCost = () => {
        return Math.round((payItem.item.price + payItem.option.addPrice) * 0.9 * amount) + handleDeliverCost();
    }
            ...
            {payItem !== undefined && Object.keys(payItem).length > 0 ? (
                <>
                    ...
                    <tr>
                        <td rowSpan={2}>
                            <img
                                src={`${process.env.REACT_APP_IMG_PUBLIC_URI}/03_upload/${payItem.item.img.split("/")[0]}`}
                                className="btn_User_Pay_Img"/>
                        </td>
                        <td colSpan={2}>
                            {payItem.item.content}
                        </td>
                    </tr>
                    <tr>
                        <td></td>
                        <td>
                            옵션 : {payItem.option.optionInfo}
                        </td>
                        <td>
                            개수 : {amount} 개
                        </td>
                            ...
                            <tr>
                                ...
                                <table className="User_Pay_Cash">
                                    <tr>
                                        ...
                                        <td>
                                            {`${Math.round((payItem.item.price + payItem.option.addPrice) * 0.9 * amount)} 원 (공동구매 10% 할인)`}
                                        </td>
                                    </tr>
                                    {/* <tr>
                                        <td>
