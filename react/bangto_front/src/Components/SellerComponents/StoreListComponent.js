...
const StoreListComponent = (props) => {
    ...
    const handleItemDelete = async (id) => {
        if(window.confirm("물품을 삭제하시겠습니까?")) {
            try{
                const response = await axios.post(`${SERVER_PORT}/item/delete`,{id, storePk: props.storeInfo.id},{
                    headers: {
    ...
    const handleItemList = async () => {
        try{
            ...
            if(props.storeInfo && props.storeInfo.id){
                const itemResponse = await axios.get(`${SERVER_PORT}/item/get-itemlist/${props.storeInfo.id}/${itemPage}`,{
                    headers: {
                      "Authorization": `Bearer ${localStorage.getItem("token")}`,
                    }
                  })
    ...
    const handleOptionDelete = async (optionId) => {
        try{
            const response = await axios.post(`${SERVER_PORT}/item/option/delete`,{id: optionId},{
                headers: {
                    "Authorization": `Bearer ${localStorage.getItem("token")}`,
                }
            })
    ...
    const handleOptionAdd = async (itemPk) => {
        try{
            const response = await axios.post(`${SERVER_PORT}/item/option/add`,
    ...
    return (
        <>
                        ...
                        { itemlist.map((item) => (
                            ...
                            {Array.isArray(item.options) && item.options.length > 0 ? (
                                <>
                                    ...
                                    {item.options.map((ele, ind) => <option value={ind}>{ele.optionInfo}</option>)}
                                    </select>
                                    {option && option != -1 ? 
                                    <>
                                        <div>{item.options[option].addPrice}원  {item.options[option].amount}개</div>
                                        <div>
                                            <button onClick={()=> handleOptionDelete(item.options[option].id)}>삭제</button>
                                            <button onClick={()=> toggleAddOptionView()}>추가</button>
                                        </div>
                                        {addOptionView ? 
                                            <div>
                                                <input placeholder='옵션 정보' onChange={(e)=> setOptionInfo(e.target.value)}></input>
                                                <input placeholder='옵션 가격' onChange={(e)=> setOptionPrice(e.target.value)}></input>
                                                <input placeholder='옵션 수량' onChange={(e)=> setOptionAmount(e.target.value)}></input>
                                                <button onClick={()=> handleOptionAdd(item.id)}>등록</button>
                                            </div>
                                            : null
                                        }
                                    </>: <div></div>}
                            ...
                            <td>
                            <button
                                onClick={() => {setSelectedItem(item); setGroupItemModal(true);}}>
                                공동구매 등록
                            </button>
                            </td>
                            <td>
            ...
            <StoreGroupItemRegisterComponent modal={groupItemModal} setModal={setGroupItemModal} store={props.storeInfo} sellerName={props.sellerName} item={selectedItem}/>
            <StoreItemRegisterComponent modal={addItemModal} setModal={setAddItemModal} store={props.storeInfo} sellerName={props.sellerName}/>
            <StoreAddComponent modal={addStoreModal} setModal={setAddStoreModal} store={props.storeInfo} sellerName={props.sellerName} />
            <StoreModiComponent modal={modiModal} setModal={setModiModal} store={props.storeInfo} sellerName={props.sellerName}/>
        </>
    );
}
